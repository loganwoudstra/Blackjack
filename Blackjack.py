def restart():
    ...
    def main():
        # clears users and dealers deck the dealer and player each have 'cards' and 'face', with 'cards' being the
        # value of the their hand, and 'face' being the representation of their hand(ie 'face' has face cards as a
        # number while 'cards' has face cards as their corresponding numeric value)
        dealer_cards = []
        ...
        def playagain():  # whether player wants to play another game
            ...
            if play == "n" or play == "no":  # player doesnt want to play again
                print("Chips: " + str(chips))
                exit()
            else:  # player inputs an invalid response(not 'y','yes','n', or 'no'
                print("Invalid response")
                playagain()
        ...
        def player_score():  # determines the players current score
            ...
            if total == 21:  # players hits blackjack
                ...
                if 11 in player_cards:  # if player has an ace, it value changes from 11 to 1
                    ...
                    return win(-1)
            else:  # if player doesnt bust or hit blackjack, the sum of their cards are returned to be compared to
                # the dealers
                return sum(player_cards)
        ...
        def dealer_score():  # determines the dealers current score
            ...
            if total > 21:  # if dealer busts
                ...
                return sum(
                    dealer_cards)  # if dealer doesnt bust or hit blackjack, the sum of their cards are returned to
                # be compared to the players

        def compare(player_total, dealer_total):  # determines who wins between dealer and player
            if player_total > dealer_total:  # player has a higher score and wins
                return win(1)
        ...
        def draw(list, list2):  # draws a new card at random
            card = random.randint(1, 13)  # picks number between 1 and 13, and if the card is a face card it is assigned
